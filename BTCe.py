...
class API:
    ...
    def transhistory(self, from_=0, count=1000, fromid=0, endid=sys.maxsize, order='DESC', since=0, end=sys.maxsize):
        ...
        return self.request('TransHistory',
                            {'from': from_, 'count': count, 'from_id': fromid, 'end_id': endid, 'order': order,
                             'since': since, 'end': end})

    def tradehistory(self, from_=0, count=1000, fromid=0, endid=sys.maxsize, order='DESC', since=0, end=sys.maxsize,
                     pair='', active=1):
        """Request trade history."""
        return self.request('TradeHistory',
                            {'from': from_, 'count': count, 'from_id': fromid, 'end_id': endid, 'order': order,
                             'since': since, 'end': end, 'pair': pair, 'active': active})
    ...
    def query(method, pair=''):
        ...
        response = ''
        try:
            response = urllib.request.urlopen('http://btc-e.com/api/3/{method}/{pair}'.format(method=method, pair=pair),
