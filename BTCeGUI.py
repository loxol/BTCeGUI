...
class TradeFrame(ttk.Frame):
    ...
    def __init__(self, parent, type):
        ...
        self.currentries = [ttk.Entry(self, justify='right', validate='key', validatecommand=validatecommand,
                                      textvariable=self.currvars[i]) for i in range(2)]
        self.currlabels = [ttk.Label(self, text='') for i in range(2)]
        self.rateentry = ttk.Entry(self, justify='right', validate='key', validatecommand=validatecommand,
                                   textvariable=self.ratevar)
        self.feeentry = ttk.Entry(self, justify='right', state='readonly', validate='key',
                                  validatecommand=validatecommand, textvariable=self.feevar)
        ...
        ttk.Checkbutton(self, text='All', variable=self.allchecked, command=self.update_amounts).grid(column=1, row=5,
                                                                                                      sticky='nw')
...
class OrderFrame(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        # init widgets
        self.table = ttk.Treeview(self, columns=['id', 'time', 'pair', 'type', 'rate', 'amount', 'value', 'status'],
...
class Main(tkinter.Tk):
    ...
    def __init__(self):
        ...
        # events
        self.askframe.table.bind('<Double-1>',
                                 lambda event: self.ondouble_depth(self.askframe.table, self.buybox, event))
        self.bidframe.table.bind('<Double-1>',
                                 lambda event: self.ondouble_depth(self.bidframe.table, self.sellbox, event))
